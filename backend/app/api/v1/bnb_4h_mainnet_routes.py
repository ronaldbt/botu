...
async def start_bnb_4h_scanner(
    ...
    try:
        logger.info(f"👤 Usuario {current_user.id} ({current_user.username}) iniciando scanner BNB 4h Mainnet")
        ...
        if bnb_scanner.is_running:
            return {
                "success": False,
                "message": "El scanner BNB 4h Mainnet ya está ejecutándose",
        ...
        logger.info("✅ Scanner BNB 4h Mainnet iniciado exitosamente")
        return {
            "success": True,
        ...
        logger.error(f"❌ Error iniciando scanner BNB 4h Mainnet: {e}")
...
async def stop_bnb_4h_scanner(
    ...
    try:
        logger.info(f"👤 Usuario {current_user.id} ({current_user.username}) deteniendo scanner BNB 4h Mainnet")
        ...
        logger.info("⏹️ Scanner BNB 4h Mainnet detenido exitosamente")
        return {
            "success": True,
        ...
        logger.error(f"❌ Error deteniendo scanner BNB 4h Mainnet: {e}")
...
async def get_bnb_4h_scanner_status(
    ...
    try:
        ...
        logger.error(f"❌ Error obteniendo estado del scanner BNB 4h Mainnet: {e}")
...
async def get_bnb_4h_scanner_logs(
    ...
    try:
        ...
        logger.error(f"❌ Error obteniendo logs del scanner BNB 4h Mainnet: {e}")
    ...
    try:
        ...
        if not enabled_keys:
            ...
            if candidate:
                ...
                db.commit()
                auto_enabled = True
                logger.info(f"🟢 Habilitada BNB 4h Mainnet en API key {candidate.id} para usuario {current_user.id}")
        ...
        logger.info(f"🧪 Forzando compra BNB 4h Mainnet con precio ${price:.2f}")
...
async def test_bnb_4h_mainnet_scan(
    ...
    try:
        logger.info(f"👤 Usuario {current_user.id} ({current_user.username}) ejecutando escaneo de prueba BNB 4h Mainnet")
        ...
        logger.info("✅ Escaneo de prueba BNB 4h Mainnet completado")
        return {
            "success": True,
        ...
        logger.error(f"❌ Error en escaneo de prueba BNB 4h Mainnet: {e}")
...
async def get_bnb_4h_scanner_config(
    ...
    """Obtiene la configuración del scanner BNB 4h Mainnet"""
    try:
        ...
        logger.error(f"❌ Error obteniendo configuración del scanner BNB 4h Mainnet: {e}")
...
async def update_bnb_4h_scanner_config(
    ...
    """Actualiza la configuración del scanner BNB 4h Mainnet"""
    try:
        logger.info(f"👤 Usuario {current_user.id} ({current_user.username}) actualizando configuración scanner BNB 4h Mainnet")
        ...
        logger.info("✅ Configuración del scanner BNB 4h Mainnet actualizada")
        return {
            "success": True,
        ...
        logger.error(f"❌ Error actualizando configuración del scanner BNB 4h Mainnet: {e}")
...
async def get_bnb_4h_scanner_alerts(
    ...
    try:
        ...
        logger.error(f"❌ Error obteniendo alertas del scanner BNB 4h Mainnet: {e}")
...
async def get_bnb_4h_current_price_mainnet(
    ...
    try:
        ...
        logger.error(f"❌ Error obteniendo precio actual de BNB 4h Mainnet: {e}")
...
async def get_bnb_4h_mainnet_positions(
    ...
    try:
        ...
        logger.info(f"📊 Obteniendo posiciones BNB 4h Mainnet para usuario {current_user.id}")
        ...
        for api_key in api_keys:
            # Buscar órdenes de compra ejecutadas para BNBUSDT (solo las que NO están completadas)
            ...
            for buy_order in buy_orders:
                ...
                if not sell_order:
                    ...
                    logger.info(f"📊 Posición abierta encontrada: {position['quantity']:.6f} BNB @ ${position['entry_price']:.2f}")
        ...
        logger.error(f"❌ Error obteniendo posiciones BNB 4h Mainnet: {e}")
...
async def get_bnb_4h_scanner_performance(
    ...
    """Obtiene métricas de rendimiento del scanner BNB 4h Mainnet"""
    try:
        status_data = bnb_scanner.get_status()
        ...
        logger.error(f"❌ Error obteniendo rendimiento del scanner BNB 4h Mainnet: {e}")
