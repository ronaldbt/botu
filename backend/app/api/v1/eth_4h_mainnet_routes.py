# backend/app/api/v1/eth_4h_mainnet_routes.py
...
# Crear router
router = APIRouter(prefix="/trading/scanner/eth-4h-mainnet", tags=["eth-4h-mainnet-scanner"])

# Instanciar ejecutor específico
eth_4h_executor = AutoTradingEth4hExecutor()

# --------------------------
# Control del Scanner ETH 4h Mainnet
# --------------------------

@router.post("/start")
async def start_eth_4h_scanner(
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Inicia el scanner de ETH 4h para Mainnet"""
    try:
        logger.info(f"👤 Usuario {current_user.id} ({current_user.username}) iniciando scanner ETH 4h Mainnet")
        ...
        if eth_scanner.is_running:
            return {
                "success": False,
                "message": "El scanner ETH 4h Mainnet ya está ejecutándose",
                "status": eth_scanner.get_status()
            }
        
        # Iniciar scanner
        import asyncio
        asyncio.create_task(eth_scanner.start_scanner())
        
        logger.info("✅ Scanner ETH 4h Mainnet iniciado exitosamente")
        return {
            "success": True,
            "message": "Scanner ETH 4h Mainnet iniciado exitosamente",
            "status": eth_scanner.get_status()
        }
            
    except Exception as e:
        logger.error(f"❌ Error iniciando scanner ETH 4h Mainnet: {e}")
        raise HTTPException(status_code=500, detail=f"Error iniciando scanner: {str(e)}")

@router.post("/stop")
async def stop_eth_4h_scanner(
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Detiene el scanner de ETH 4h Mainnet"""
    try:
        logger.info(f"👤 Usuario {current_user.id} ({current_user.username}) deteniendo scanner ETH 4h Mainnet")
        ...
        await eth_scanner.stop_scanner()
        
        logger.info("⏹️ Scanner ETH 4h Mainnet detenido exitosamente")
        return {
            "success": True,
            "message": "Scanner ETH 4h Mainnet detenido exitosamente",
            "status": eth_scanner.get_status()
        }
            
    except Exception as e:
        logger.error(f"❌ Error deteniendo scanner ETH 4h Mainnet: {e}")
        raise HTTPException(status_code=500, detail=f"Error deteniendo scanner: {str(e)}")

@router.get("/status")
async def get_eth_4h_scanner_status(
    current_user: User = Depends(get_current_user)
):
    """Obtiene el estado actual del scanner ETH 4h Mainnet"""
    try:
        ...
        logger.error(f"❌ Error obteniendo estado del scanner ETH 4h Mainnet: {e}")
        raise HTTPException(status_code=500, detail=f"Error obteniendo estado: {str(e)}")

@router.get("/logs")
async def get_eth_4h_scanner_logs(
    current_user: User = Depends(get_current_user),
    limit: int = 100
):
    """Obtiene los logs del scanner ETH 4h Mainnet"""
    try:
        ...
        logger.error(f"❌ Error obteniendo logs del scanner ETH 4h Mainnet: {e}")
...
async def force_buy_eth_4h_mainnet(
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    ...
    try:
        ...
        # Asegurar que el usuario tenga al menos una API key mainnet habilitada para ETH 4h
        ...
        auto_enabled = False
        if not enabled_keys:
            # Buscar alguna key mainnet activa del usuario y habilitarla para ETH 4h
            ...
            if candidate:
                candidate.eth_4h_mainnet_enabled = True
                db.commit()
                auto_enabled = True
                logger.info(f"🟢 Habilitada ETH 4h Mainnet en API key {candidate.id} para usuario {current_user.id}")
        ...
        logger.info(f"🧪 Forzando compra ETH 4h Mainnet con precio ${price:.2f}")
        await eth_4h_executor.execute_buy_order(fake_signal, user_id=current_user.id)
...
async def test_eth_4h_mainnet_scan(
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Ejecuta un escaneo de prueba del scanner ETH 4h Mainnet"""
    try:
        logger.info(f"👤 Usuario {current_user.id} ({current_user.username}) ejecutando escaneo de prueba ETH 4h Mainnet")
        ...
        await eth_scanner._scan_cycle()
        
        logger.info("✅ Escaneo de prueba ETH 4h Mainnet completado")
        return {
            "success": True,
        ...
        logger.error(f"❌ Error en escaneo de prueba ETH 4h Mainnet: {e}")
        raise HTTPException(status_code=500, detail=f"Error en escaneo de prueba: {str(e)}")

@router.get("/config")
async def get_eth_4h_scanner_config(
    current_user: User = Depends(get_current_user)
):
    """Obtiene la configuración del scanner ETH 4h Mainnet"""
    try:
        config = eth_scanner.config.copy()
        detection_params = eth_scanner.detection_params.copy()
        ...
        logger.error(f"❌ Error obteniendo configuración del scanner ETH 4h Mainnet: {e}")
        raise HTTPException(status_code=500, detail=f"Error obteniendo configuración: {str(e)}")

@router.put("/config")
async def update_eth_4h_scanner_config(
    config_updates: Dict[str, Any],
    current_user: User = Depends(get_current_user)
):
    """Actualiza la configuración del scanner ETH 4h Mainnet"""
    try:
        logger.info(f"👤 Usuario {current_user.id} ({current_user.username}) actualizando configuración scanner ETH 4h Mainnet")
        ...
        if 'detection_params' in config_updates:
            eth_scanner.detection_params.update(config_updates['detection_params'])
        
        logger.info("✅ Configuración del scanner ETH 4h Mainnet actualizada")
        return {
            "success": True,
            "message": "Configuración actualizada exitosamente",
            "config": eth_scanner.config,
            "detection_params": eth_scanner.detection_params
        }
            
    except Exception as e:
        logger.error(f"❌ Error actualizando configuración del scanner ETH 4h Mainnet: {e}")
        raise HTTPException(status_code=500, detail=f"Error actualizando configuración: {str(e)}")

@router.get("/alerts")
async def get_eth_4h_scanner_alerts(
    current_user: User = Depends(get_current_user),
    limit: int = 50
):
    """Obtiene las alertas generadas por el scanner ETH 4h Mainnet"""
    try:
        ...
        logger.error(f"❌ Error obteniendo alertas del scanner ETH 4h Mainnet: {e}")
        raise HTTPException(status_code=500, detail=f"Error obteniendo alertas: {str(e)}")

@router.get("/current-price")
async def get_eth_4h_current_price_mainnet(
    current_user: User = Depends(get_current_user)
):
    ...
    try:
        ...
        logger.error(f"❌ Error obteniendo precio actual de ETH 4h Mainnet: {e}")
        raise HTTPException(status_code=500, detail=f"Error obteniendo precio: {str(e)}")

@router.get("/positions")
async def get_eth_4h_mainnet_positions(
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Obtiene las posiciones abiertas de ETH 4h Mainnet"""
    try:
        from app.db.models import TradingOrder, TradingApiKey
        from sqlalchemy import and_
        
        logger.info(f"📊 Obteniendo posiciones ETH 4h Mainnet para usuario {current_user.id}")
        ...
        for api_key in api_keys:
            # Buscar órdenes de compra ejecutadas para ETHUSDT (solo las que NO están completadas)
        ...
        logger.error(f"❌ Error obteniendo posiciones ETH 4h Mainnet: {e}")
...
async def get_eth_4h_scanner_performance(
    current_user: User = Depends(get_current_user)
):
    """Obtiene métricas de rendimiento del scanner ETH 4h Mainnet"""
    try:
        status_data = eth_scanner.get_status()
        ...
        logger.error(f"❌ Error obteniendo rendimiento del scanner ETH 4h Mainnet: {e}")
