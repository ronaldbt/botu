...
class EthScannerService:
    ...
    async def _scan_loop(self):
        ...
        while self.is_running:
            try:
                ...
                logger.error(f"❌ Error en loop de escaneo ETH: {e}")
    ...
    async def _get_binance_data(self) -> Optional[pd.DataFrame]:
        """Obtiene datos históricos de Binance para ETH"""
        try:
            ...
            logger.error(f"❌ Error obteniendo datos ETH de Binance: {e}")
            return None
    
    def _detect_u_pattern_2022(self, df: pd.DataFrame) -> List[Dict]:
        """
        Detecta patrones U usando la lógica exitosa del backtest 2022 para ETH
        """
        signals = []
        ...
        for end_idx in range(window_size, len(df)):
            ...
            for low in lows[-3:]:  # Solo últimos 3 mínimos
                ...
                if all(conditions):
                    ...
                    logger.info(f"🎯 ETH PATRÓN U DETECTADO - FILTROS ESTRICTOS PASADOS:")
