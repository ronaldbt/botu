...
def scan_for_u_bnb(ticker, klines_data, verbose=False):
    """
    Scanner de patrones U optimizado específicamente para BNB
    Basado en la estrategia exitosa de bnb_2023_backtest.py
    
    Args:
        ticker: Símbolo del ticker (ej: "BNBUSDT")
    ...
    """
    if verbose:
        print(f"[{ticker}] 🪙 Usando scanner BNB optimizado - {len(klines_data)} velas...")
    ...
    try:
        ...
        if len(df) < 120:  # Necesitamos al menos 120 velas para el análisis BNB
            if verbose:
                print(f"[{ticker}] ❌ No hay suficientes datos para análisis BNB (necesario: 120, actual: {len(df)})")
            return {
                "alert": False,
                "nivel_ruptura": None,
                ...
                "strategy": "BNB_OPTIMIZED"
            }
        
        # Detectar señales usando algoritmo BNB optimizado
        signals = _detect_u_patterns_bnb(df, verbose=verbose)
        
        if not signals:
            if verbose:
                print(f"[{ticker}] No se detectaron patrones U con criterios BNB")
            return {
                "alert": False,
                "nivel_ruptura": None,
                "precio_confirmacion": df['close'].iloc[-1],
                "slope_left": None,
                "estado_sugerido": "NO_U_BNB",
                "strategy": "BNB_OPTIMIZED"
            }
        ...
        if precio_actual > best_signal['entry_price']:
            if verbose:
                print(f"[{ticker}] ✅ PATRÓN U BNB DETECTADO Y ROTO!")
                print(f"[{ticker}] Nivel BNB: {best_signal['entry_price']:.4f}")
            ...
            if verbose:
                print(f"[{ticker}] ⚠️ Patrón U BNB detectado pero no roto")
                print(f"[{ticker}] Nivel BNB: {best_signal['entry_price']:.4f}")
                print(f"[{ticker}] Precio actual: {precio_actual:.4f}")
            ...
            return {
                ...
                "precio_confirmacion": precio_actual,
                "slope_left": best_signal['signal_strength'],
                "estado_sugerido": "U_BNB_DETECTADO",
                "strategy": "BNB_OPTIMIZED",
                "signal_strength": best_signal['signal_strength'],
                "depth": best_signal['depth']
            }
        ...
        logger.error(f"Error en scan_for_u_bnb para {ticker}: {e}")
        return {
            "alert": False,
            "nivel_ruptura": None,
...
def _detect_u_patterns_bnb(df, verbose=False):
    """
    Detecta patrones U optimizado para BNB (menos volátil que BTC/ETH)
    Basado en la estrategia exitosa de bnb_2023_backtest.py
    """
    signals = []
    
    # Detectar mínimos significativos optimizados para BNB
    significant_lows = _detect_lows_bnb(df, window=6, min_depth_pct=0.025, verbose=verbose)  # 2.5% mínimo (BNB menos volátil)
    
    if not significant_lows:
        if verbose:
            print(f"   No se encontraron mínimos significativos para BNB")
        return signals
    ...
    for low in significant_lows[-3:]:  # Últimos 3 mínimos
        ...
        # Factor optimizado para BNB (más conservador)
        dynamic_factor = _calculate_rupture_factor_bnb(atr, current_price)
        nivel_ruptura = low['high'] * dynamic_factor
        
        # Condiciones optimizadas para BNB
        if len(df) - min_idx > 4 and len(df) - min_idx < 45:
            ...
            conditions = [
                ...
                _check_momentum_filter_bnb(df, min_idx)
            ]
            ...
            if all(conditions):
                ...
                if verbose:
                    print(f"   ✨ Señal BNB válida encontrada:")
...
def _detect_lows_bnb(df, window=6, min_depth_pct=0.025, verbose=False):
    """Detecta mínimos optimizados para BNB"""
    lows = []
    ...
    if verbose and lows:
        print(f"   Mínimos BNB detectados: {len(lows)} (profundidad min: {min_depth_pct*100:.1f}%)")
    
    return lows

def _calculate_rupture_factor_bnb(atr, price, base_factor=1.015):
    """Factor de ruptura optimizado para BNB (más conservador que BTC/ETH)"""
    atr_pct = atr / price
...
def _check_momentum_filter_bnb(df, min_idx):
