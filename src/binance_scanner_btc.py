...
def scan_for_u_btc(ticker, klines_data, verbose=False):
    """
    Scanner de patrones U optimizado específicamente para BTC
    Basado en la estrategia exitosa de bitcoin_2023_backtest.py (versión conservadora)
    ...
    """
    if verbose:
        print(f"[{ticker}] ₿ Usando scanner BTC optimizado - {len(klines_data)} velas...")
    ...
    try:
        ...
        if len(df) < 120:  # Necesitamos al menos 120 velas para el análisis BTC
            if verbose:
                print(f"[{ticker}] ❌ No hay suficientes datos para análisis BTC (necesario: 120, actual: {len(df)})")
            return {
                "alert": False,
                "nivel_ruptura": None,
                ...
                "strategy": "BTC_OPTIMIZED"
            }
        
        # Detectar señales usando algoritmo BTC optimizado
        signals = _detect_u_patterns_btc(df, verbose=verbose)
        
        if not signals:
            if verbose:
                print(f"[{ticker}] No se detectaron patrones U con criterios BTC")
            return {
                "alert": False,
                "nivel_ruptura": None,
                "precio_confirmacion": df['close'].iloc[-1],
                "slope_left": None,
                "estado_sugerido": "NO_U_BTC",
                "strategy": "BTC_OPTIMIZED"
            }
        ...
        if precio_actual > best_signal['entry_price']:
            if verbose:
                print(f"[{ticker}] ✅ PATRÓN U BTC DETECTADO Y ROTO!")
                print(f"[{ticker}] Nivel BTC: {best_signal['entry_price']:.4f}")
            ...
            if verbose:
                print(f"[{ticker}] ⚠️ Patrón U BTC detectado pero no roto")
                print(f"[{ticker}] Nivel BTC: {best_signal['entry_price']:.4f}")
                print(f"[{ticker}] Precio actual: {precio_actual:.4f}")
            ...
            return {
                ...
                "precio_confirmacion": precio_actual,
                "slope_left": best_signal['signal_strength'],
                "estado_sugerido": "U_BTC_DETECTADO",
                "strategy": "BTC_OPTIMIZED",
                "signal_strength": best_signal['signal_strength'],
                "depth": best_signal['depth']
            }
        ...
        logger.error(f"Error en scan_for_u_btc para {ticker}: {e}")
        return {
            "alert": False,
            "nivel_ruptura": None,
...
def _detect_u_patterns_btc(df, verbose=False):
    """
    Detecta patrones U optimizado para BTC (versión conservadora actualizada)
    Basado en la estrategia de bitcoin_2023_backtest.py modificado
    """
    signals = []
    
    # Detectar mínimos significativos optimizados para BTC (ahora más conservador)
    significant_lows = _detect_lows_btc(df, window=6, min_depth_pct=0.025, verbose=verbose)  # 2.5% mínimo
    
    if not significant_lows:
        if verbose:
            print(f"   No se encontraron mínimos significativos para BTC")
        return signals
        
    # Analizar múltiples mínimos (4 como en el backtest original, pero con filtros conservadores)
    for low in significant_lows[-4:]:  # Últimos 4 mínimos (más oportunidades que BNB/ETH)
        min_idx = low['index']
        ...
        # Factor conservador para BTC (ahora similar a BNB/ETH)
        dynamic_factor = _calculate_rupture_factor_btc(atr, current_price)
        nivel_ruptura = low['high'] * dynamic_factor
        
        # Condiciones optimizadas para BTC (ahora más conservadoras)
        if len(df) - min_idx > 4 and len(df) - min_idx < 45:
            ...
            # Condiciones conservadoras para BTC (alineadas con la versión actualizada)
            conditions = [
                pre_slope < -0.12,  # Más restrictivo (igual que BNB/ETH)
                ...
                _check_momentum_filter_btc(df, min_idx)
            ]
            ...
            if all(conditions):
                ...
                if verbose:
                    print(f"   ✨ Señal BTC válida encontrada:")
...
def _detect_lows_btc(df, window=6, min_depth_pct=0.025, verbose=False):
    """Detecta mínimos optimizados para BTC (con filtros de volumen)"""
    lows = []
    ...
    if verbose and lows:
        print(f"   Mínimos BTC detectados: {len(lows)} (profundidad min: {min_depth_pct*100:.1f}%)")
    
    return lows

def _calculate_rupture_factor_btc(atr, price, base_factor=1.015):
    """
    Factor de ruptura optimizado para BTC (ahora conservador como BNB/ETH)
    Basado en bitcoin_2023_backtest.py actualizado
    """
    atr_pct = atr / price
    
    # Conservador para BTC (ahora alineado con BNB/ETH)
    if atr_pct < 0.015:
        factor = base_factor
    elif atr_pct < 0.03:
...
def _check_momentum_filter_btc(df, min_idx):
