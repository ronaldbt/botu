...
const regenerateToken = async () => {
  ...
  try {
    const response = await apiClient.post('/telegram/regenerate-token?crypto=bnb', {}, {
      headers: {
        'Authorization': `Bearer ${authStore.token}`
      }
    })
    ...
    console.log('Token BNB regenerated:', response.data)
  } catch (error) {
    console.error('Error regenerando token BNB:', error)
  } finally {
