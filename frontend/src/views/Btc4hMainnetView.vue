  ...
  <div class="min-h-screen bg-slate-50">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between py-4">
          <div class="flex items-center space-x-4">
            <div class="flex items-center space-x-3">
              ...
              <div>
                <h1 class="text-xl font-bold text-slate-900">BTC 4h Scanner</h1>
            ...
            <div class="flex items-center space-x-4">
              <div class="flex items-center space-x-2">
                ...
                <span class="text-sm font-medium text-green-600">
                  {{ btc4hScanner.scannerStatus.value.is_running ? 'Scanner Activo' : 'Scanner Inactivo' }}
                </span>
              </div>
              ...
              <div class="text-xs text-slate-400">
                Último escaneo: {{ btc4hScanner.scannerStatus.value.last_scan_time || '—' }}
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div class="space-y-8">
          ...
          <div class="p-6">
            ...
            <div v-else class="space-y-6">
              ...
              >
                ...
                <!-- BTC 4h Mainnet Configuration -->
                <div class="bg-white rounded-lg p-4 border border-orange-200">
                  <div class="flex items-center justify-between mb-3">
                    <div class="flex items-center space-x-2">
                      <span class="text-orange-500 text-lg">₿</span>
                      <span class="font-semibold text-slate-800">BTC 4h Scanner</span>
                    </div>
                    <button
                      @click="toggleBtc4hMainnet(apiKey)"
                      :class="[
                        'px-4 py-2 rounded-lg text-sm font-medium transition-colors',
                        apiKey.btc_4h_mainnet_enabled 
                          ? 'bg-green-500 text-white hover:bg-green-600' 
                          : 'bg-slate-400 text-white hover:bg-slate-500'
                      ]"
                    >
                      {{ apiKey.btc_4h_mainnet_enabled ? '✓ Activado' : 'Activar Scanner' }}
                    </button>
                  </div>
                  <div class="flex items-center gap-3 mb-3">
                    <button
                      @click="handleForceBuy"
                      :disabled="!btc4hScanner.scannerStatus.value?.auto_trading_readiness?.auto_ready"
                      class="px-3 py-2 text-white text-xs rounded"
                      :class="btc4hScanner.scannerStatus.value?.auto_trading_readiness?.auto_ready ? 'bg-orange-600 hover:bg-orange-700' : 'bg-slate-400'"
                    >
                      Forzar compra (simulación)
                    </button>
                    <span class="text-xs" :class="btc4hScanner.scannerStatus.value?.auto_trading_readiness?.auto_ready ? 'text-slate-500' : 'text-red-500'">
                      {{ btc4hScanner.scannerStatus.value?.auto_trading_readiness?.auto_ready ? 'Prueba en MAINNET con asignación actual' : 'Auto-trading NO LISTO: ' + (btc4hScanner.scannerStatus.value?.auto_trading_readiness?.reasons?.join(', ') || '') }}
                    </span>
                  </div>
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        type="number"
                        :value="apiKey.btc_4h_mainnet_allocated_usdt || 0"
                        @change="updateBtc4hMainnetAllocation(apiKey, $event.target.value)"
                        min="0"
                        step="0.01"
                    ...
                    <div class="flex items-end">
                      <div class="text-sm text-slate-600">
                        ...
                        <div class="text-orange-600">
                          ${{ (apiKey.btc_4h_mainnet_allocated_usdt || 0).toFixed(2) }} USDT asignados
                        </div>
                      </div>
                    </div>
                  </div>
        ...
        <CryptoStatusCards
          :scanner-status="btc4hScanner.scannerStatus.value"
          :current-price="currentBtcPrice"
          crypto-name="BTC"
          crypto-symbol="BTC"
          crypto-emoji="₿"
          timeframe="4h"
        ...
        <!-- Scanner Logs -->
        <CryptoScannerLogs
          :logs="btc4hScanner.scannerLogs.value"
          :refreshing="btc4hScanner.refreshingLogs.value"
          :scanner-status="btc4hScanner.scannerStatus.value"
          :current-price="currentBtcPrice"
          crypto-name="BTC"
          crypto-symbol="BTC"
          crypto-slug="btc-4h"
          timeframe="4h"
          @refresh="btc4hScanner.refreshLogs"
        />
      </div>
    </div>
...
import { useBtc4hMainnetScanner } from '@/composables/useBtc4hMainnetScanner'
...
const btc4hScanner = useBtc4hMainnetScanner()

// Local state
const showAddApiKey = ref(false)
const showApiHelpModal = ref(false)
const portfolioRef = ref(null)
const currentBtcPrice = ref(115000)
...
const mainnetOrders = computed(() => {
  ...
  return envFiltered.filter(order => 
    order.crypto_symbol === 'BTC' || 
    (order.ticker === 'BTCUSDT') ||
...
const loadCurrentBtcPrice = async () => {
  try {
    ...
    await btc4hScanner.refreshStatus()
    const statusPrice = btc4hScanner.scannerStatus.value?.btc_price
    if (statusPrice) {
      currentBtcPrice.value = statusPrice
      console.log('[Btc4hMainnet] Precio BTC (status/last_scan):', currentBtcPrice.value)
    } else {
      const response = await apiClient.get('/trading/scanner/btc-4h-mainnet/current-price')
      if (response.data.success) {
        currentBtcPrice.value = response.data.price
        console.log('[Btc4hMainnet] Precio BTC (endpoint directo):', currentBtcPrice.value)
      }
    }
  } catch (error) {
...
const loadData = async () => {
  console.log('[Btc4hMainnet] loadData() inicio')
  try {
    await Promise.all([
      apiKeys.loadApiKeys(),
      orders.loadOrders(),
      btc4hScanner.initializeScanner(),
      loadCurrentBtcPrice()
    ])
    
    // Log detallado de las API keys después de cargar
    console.log('[Btc4hMainnet] 🔑 API Keys cargadas:', {
      ...
      mainnetKeysDetail: mainnetApiKeys.value.map(k => ({
        ...
        btc_4h_mainnet_enabled: k.btc_4h_mainnet_enabled,
        btc_4h_mainnet_allocated_usdt: k.btc_4h_mainnet_allocated_usdt
      }))
    })
    ...
    console.log('[Btc4hMainnet] loadData() completado')
  } catch (error) {
    console.error('Error cargando datos BTC 4h Mainnet:', error)
  }
}

const toggleBtc4hMainnet = async (apiKey) => {
  const enabled = !apiKey.btc_4h_mainnet_enabled
  await updateBtc4hMainnetAllocation(apiKey, apiKey.btc_4h_mainnet_allocated_usdt || 0, enabled)
}

const updateBtc4hMainnetAllocation = async (apiKey, amount, enabled = null) => {
  try {
    const updateData = {
      crypto: 'btc_4h_mainnet',
      enabled: enabled !== null ? enabled : apiKey.btc_4h_mainnet_enabled,
      allocated_usdt: parseFloat(amount) || 0
    }
    
    console.log('[Btc4hMainnet] 🔧 Actualizando BTC 4h allocation:', {
      apiKeyId: apiKey.id,
      currentEnabled: apiKey.btc_4h_mainnet_enabled,
      currentAllocated: apiKey.btc_4h_mainnet_allocated_usdt,
      newAmount: amount,
      newEnabled: enabled,
    ...
    console.log('[Btc4hMainnet] ✅ Respuesta del servidor:', response.data)
    
    await apiKeys.loadApiKeys()
    
    console.log('[Btc4hMainnet] BTC 4h mainnet allocation actualizada exitosamente')
  } catch (error) {
    console.error('[Btc4hMainnet] ❌ Error actualizando BTC 4h mainnet allocation:', {
      error: error,
...
const handleSubmitApiKey = async () => {
  console.log('[Btc4hMainnet] Enviando API key mainnet:', { ...apiKeyForm })
  try {
...
const handleStartScanner = async () => {
  console.log('[Btc4hMainnet] Iniciando scanner...')
  const success = await btc4hScanner.startScanner()
  if (success) {
    alert('✅ Scanner BTC 4h Mainnet iniciado exitosamente')
  } else {
    alert('❌ Error iniciando scanner BTC 4h Mainnet')
  }
}

const handleStopScanner = async () => {
  console.log('[Btc4hMainnet] Deteniendo scanner...')
  const success = await btc4hScanner.stopScanner()
  if (success) {
    alert('⏹️ Scanner BTC 4h Mainnet detenido exitosamente')
  } else {
    alert('❌ Error deteniendo scanner BTC 4h Mainnet')
  }
}

const handleRefreshStatus = async () => {
  console.log('[Btc4hMainnet] Actualizando estado del scanner...')
  await btc4hScanner.refreshStatus()
}
...
const handleForceBuy = async () => {
  ...
  if (!confirmed) return
  const ok = await btc4hScanner.forceBuy()
...
watch(mainnetApiKeys, (newKeys, oldKeys) => {
  console.log('[Btc4hMainnet] 🔄 mainnetApiKeys changed:', {
...
// Lifecycle
onMounted(async () => {
  console.log('[Btc4hMainnet] onMounted - usuario:', authStore.user)
  await loadData()
  btc4hScanner.startPolling()
  // Sincronizar precio cada 45s
  const syncFromStatus = async () => {
    const p = btc4hScanner.scannerStatus.value?.btc_price
    if (p) {
      currentBtcPrice.value = p
      console.log('[Btc4hMainnet] Sync precio desde status (nuevo escaneo):', p)
    }
  }
