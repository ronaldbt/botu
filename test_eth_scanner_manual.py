#!/usr/bin/env python3
"""
Test manual del scanner ETH usando la estrategia exacta del backtest 2023
"""
...
async def test_eth_scanner_manual():
    """Prueba manual del scanner ETH con datos en vivo"""
    print("🧪 TEST MANUAL SCANNER ETHEREUM")
    print("="*60)
    print("🎯 Usando estrategia exacta del backtest ETH 2023")
    print("📊 Configuración: 8% TP | 3% SL | 2.5% depth | 120 velas")
    print("="*60)
    
    # Crear instancia del scanner
    scanner = EthScannerService()
    
    # Configurar para test manual (sin cooldown)
    scanner.cooldown_period = 0  # Sin cooldown para testing
    
    print("\n1️⃣ OBTENIENDO DATOS ACTUALES DE ETHEREUM...")
    ...
    print(f"💰 Precio actual ETH: ${df['close'].iloc[-1]:,.2f}")
    print(f"📈 Cambio 24h: {((df['close'].iloc[-1] / df['close'].iloc[-24]) - 1) * 100:+.2f}%")
    
    print("\n2️⃣ ANALIZANDO PATRONES U...")
    
    # Detectar patrones usando la lógica exacta del backtest 2023
    signals = scanner._detect_u_patterns_2023(df)
    ...
    if signals:
        ...
        significant_lows = scanner._detect_lows_2023(analysis_df)
        ...
        if significant_lows:
            ...
            dynamic_factor = scanner._calculate_rupture_factor_bull(atr, current_price)
    ...
    print(f"✅ Scanner funcionando con estrategia backtest 2023")
...
if __name__ == "__main__":
    # Ejecutar test
    asyncio.run(test_eth_scanner_manual())